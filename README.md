...
| DATE | ENTRY |
|---|---|
| 2026-05-28 | 〈Bond Overwrite Sanction · Cardinal Echo〉— 50キー Administrator 側で発覚した「無線が新規追加ペアリングできない（1台目は pair できるが2台目以降/再ペア不可）」の根本原因を双子 Cardinal へ予防同期。 真因は keymap/マクロではなく central 側 conf のセキュリティフラグ欠落: ZMK の **`CONFIG_ZMK_BLE_EXPERIMENTAL_SEC`**（公式 `docs/config/bluetooth.md` で "allows overwrite of keys from previously paired hosts" = `BT_SMP_ALLOW_UNAUTH_OVERWRITE` を imply）が無く、host 識別子に既存 bond が残るとき新規ペアリングが Zephyr SMP 層で拒否されていた。 Cardinal の `Elucidator.conf` も `CONFIG_ZMK_BLE_EXPERIMENTAL_CONN=y`（接続安定性のみ）だけで `SEC` を欠いており同症状が潜在するため、Administrator (`Night_Sky_Sword.conf`) と対称に `CONFIG_ZMK_BLE_EXPERIMENTAL_SEC=y` を host bond 管理側 central（Elucidator）へ追加（`CONN`+`SEC` で `EXPERIMENTAL_FEATURES` 相当）。 ⚠ 実機 BLE 動作は CI 検証不可（ZMK 公式ドキュメント裏付けの config 修正）。 フラッシュ後、既存 bond が不調なら `&bt BT_CLR_ALL` / settings_reset で全消去 → 再ペアが確実。 双子の無線輪が再び対称となり、複数ホストへの新規 bond 確立権能を両神器が回復。 |
