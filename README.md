...
| BT Max Conn | R: 2 / L: 1 | R・L両側 | Central=ホスト+スプリット相方の 2、Peripheral=スプリット相方のみの 1。プロファイル切替時に旧接続を強制切断しゾンビ累積を防止 |
...
| DATE | ENTRY |
|---|---|
| 2026-05-26 | 〈Phantom Connection Purge〉— Mac↔iPad プロファイル往復で時間経過と共に悪化するカーソルカクツキを二段封印。診断: `CONFIG_BT_MAX_CONN=5` が BLE 接続スロットを 5 枠開放していたため、プロファイル切替時に旧接続が即時切断されず supervision timeout (10秒) まで「ゾンビ」として残留 → 高速切替で複数の幽霊接続が Nordic 無線スケジューラから接続イベント時間を奪い合い、アクティブ接続の HID 報告が間引かれていた。修正 (a): `Elucidator.conf` (Central) の `BT_MAX_CONN=5 → 2` (ホスト 1 + スプリット相方 1)、`Dark_Repulser.conf` (Peripheral) の `BT_MAX_CONN=5 → 1` (スプリット相方のみ)。これによりプロファイル切替時に古い接続が即解放され、Nordic SoftDevice のスケジューラがアクティブ接続にフル時間を割り当て可能となる。`BT_MAX_PAIRED=5` は維持されるため 5 台ペアリング保持機能に変化なし。修正 (b): Apple 系 BLE (macOS/iPadOS/iOS) は `MIN_INT=MAX_INT=6` (7.5ms 固定要求) を範囲が狭すぎるとして拒否しデフォルト ≈ 30ms に転落する仕様のため、`BT_PERIPHERAL_PREF_MAX_INT` を 6 → 12 (15ms 上限) に緩和して 7.5〜15ms の範囲指定とし、Apple に最低 15ms を引き出す。Win/Android は引き続き 7.5ms 最速で交渉、Apple 系は 30ms → 15ms へ半減し体感カクツキを大幅軽減。〈Bilateral Sync Restoration II〉(2026-05-22) の方針通り左右両側を完全同期維持。`feature/phantom-connection-purge` ブランチで PR 化、GHA build 通過確認後に実機 BT 安定検証を経て main マージ予定。〈Noise Cancellation Re-Awakening〉(2026-05-26) で蘇生した IIR フィルタとは独立した BLE 層の浄化術式で、両者の併用により「カクツキ + 鈍さ」の二重苦から解放される設計。 |
