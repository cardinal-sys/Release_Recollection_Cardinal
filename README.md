...
| 左手(Dark Repulser) vbatt output-ohms | 443000（既定510000から補正） | 〈Fractlight Recalibration〉（2026-09-14）。左手のみ満充電（充電LED消灯確認済み）でも%が50前後で頭打ちになる症状を確認。ADC内部基準電圧・分圧抵抗の個体差により公称値(510000/1510000)とズレていたと推定し、`battery_common.c`の変換式から逆算した概算値へ補正。右手(Elucidator)は正常なため据え置き。実測（テスター）に基づかない概算補正のため、乖離が分かれば再調整が必要 |
...
| BT_PERIPHERAL_PREF_LATENCY（ホスト接続） | 未設定（Zephyr既定30） | R側（Central、ホスト接続） | ホストとの接続でデータ無しの接続イベントを何回スキップしてよいか。従来 0（毎接続イベント必須応答）だったが、〈Idle Tax Removal〉（2026-09-12）で設定ごと撤去し既定30へ委ねた。0だとホストのBLEスタックがアイドル時も休めず常時稼働し、PC側が他処理で混み合うとキー入力・トラックボール移動が両方遅延する症状の主要容疑だったが、撤去により実機で解消確認済み（アイドルからの復帰時の初動遅延等の新たな副作用も無し）。原型 Cygnus-S-Lkeymouse にはこの設定自体が存在せず既定30のまま。左右分割リンク側の `ZMK_SPLIT_BLE_PREF_LATENCY`（Split BLE Latency 行、下記）とは別物でホストとは無関係のため変更していない |
...
| PMW3610 force-awake | **実験的に無効化**（従来有効） | 〈Wakeful Vigil Suspension〉（2026-09-13）。センサーを常時RUNモード（高消費電流）に固定していた設定。原型 Cygnus-S-Lkeymouse にはこの設定自体が無く、それでも起動遅延は気にならなかったとの報告があり、Insomniaと合わせてバッテリー%変動の切り分けのため無効化。復帰時の初動遅延が気になれば本行を戻す |
...
| DATE | ENTRY |
|---|---|
| 2026-09-14 | 〈Fractlight Recalibration〉— 〈Wakeful Vigil Suspension〉(#34) マージ後も、Macの2行表示（右手＝自分の残量、左手＝`CENTRAL_BATTERY_LEVEL_PROXY`経由で中継される左手の残量、この2行表示自体はZMKの split battery proxy 機能による意図した仕様と判明・確認済み）のうち左手側が「充電しても50%前後で頭打ちになる」症状が残ることが判明。切り分けの過程で以下を全て確認したが左右差を説明する要素は見つからなかった: ①`Dark_Repulser.overlay`/`.conf` に元々バッテリー関連の記述なし ②原型 Cygnus-S-Lkeymouse の `Cygnus_L.overlay`/`.conf` も同様に記述なし・左右非対称の設定はない ③central⇔peripheral間のバッテリー値中継処理（`central.c`の`peripheral_battery_levels[]`、`central_bas_proxy.c`の`read_blvl()`）は演算を伴わない完全な素通しでバグなし ④ZMK本体のバッテリー計算コードは前回確認済みで左右共通。実機側で、左手のUSB充電中インジケーターLED（ZMKのRGB LEDウィジェットとは別、基板上の充電IC制御LED）が消灯＝充電回路的にはフル充電完了を確認できたため、実際のバッテリー電圧と、ZMKが計算している電圧の間に乖離があるキャリブレーション不一致と特定。`lithium_ion_mv_to_pct()`の変換式 `mv*2/15-459` から逆算すると、現在の50%表示は内部的に約3817.5mVとして計算されている状態。LiPo標準の満充電電圧4200mVとの比から、`xiao_ble`共通ボード定義の `&vbatt`（`output-ohms=510000`/`full-ohms=1510000`、ADC内部基準電圧・分圧抵抗の個体差を補正しない公称値）を左手のみ `output-ohms=443000`/`full-ohms=1443000` へ独自補正する `&vbatt` オーバーライドを `Dark_Repulser.overlay` に追加。右手(Elucidator)は正常なため据え置き。**注意**: テスターでの実測値ではなく、充電LED消灯（間接確認）とLiPo標準電圧の仮定に基づく概算補正のため、実測で乖離が判明すれば `output-ohms` を再調整すること。CHARACTER PARAMETERS の BATTERY SIGIL セクションに補正内容を追記。 |
