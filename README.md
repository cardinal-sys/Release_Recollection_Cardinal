...
| DATE | ENTRY |
|---|---|
| 2026-05-19 | 〈Deep Sleep Wake-up Optimization for ZMK Studio〉— Issue #3195 ワークアラウンド：ZMK Studio 有効時の BLE スリープ復帰改善。Elucidator.conf に `CONFIG_USB_DEVICE_REMOTE_WAKEUP=n`（USB リモートウェイク無効）、`CONFIG_PM_DEVICE_RUNTIME_EXCLUSIVE=n`（PM デバイス管理簡略化）、`CONFIG_PM_DEVICE_SYSTEM_MANAGED=y`（システム管理 PM 優先）、`CONFIG_BT_PERIPHERAL_PREFERRED_MTU=23`（BLE MTU 最小化）を追加。Zephyr 4.1 PM API リグレッション対策。成否はビルド後の実デバイス動作テストで検証予定。 |
