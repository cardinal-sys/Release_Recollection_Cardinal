...
| 2026-05-21 | 〈Device Tree Bindings Sealing〉— PMW3610 ドライバの `dts/bindings/pixart,pmw3610-alt.yml` に、IIR フィルタと速度ベース CPI の 7 プロパティ宣言を追加。Zephyr 4.1 device tree validation エラー「`iir-filter-alpha` is not declared in properties」を根本解決。追加プロパティ：`iir-filter-alpha` (int, default 614)、`speed-based-cpi` (boolean)、`speed-cpi-low-threshold` (int, default 5)、`speed-cpi-high-threshold` (int, default 30)、`speed-cpi-low` (int, default 1200)、`speed-cpi-medium` (int, default 2200)、`speed-cpi-high` (int, default 3200)。各プロパティに日本語注釈付き説明文も記載。driver commit `35f2c40` をリリース、`config/west.yml` revision を `e2ef34c` → `35f2c40` へ更新して GitHub Actions 全 3 ボード（Elucidator・Dark_Repulser・settings_reset）の build 成功を実現。 |
