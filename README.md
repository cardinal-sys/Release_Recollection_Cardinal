...
| DATE | ENTRY |
|---|---|
| 2026-09-23 | 〈Sigil Glyph Restoration〉— ユーザーから SIGN レイヤー（記号レイヤー）の `%` 等が `keymap.svg` の記憶図に映っていないとの指摘を受け調査。原因は `keymap_drawer.yaml` の `raw_binding_map` 全項目に `&` プレフィックスが欠落しており、`keymap-drawer`（v0.21.0、ソース直接確認済み）のマッチング条件（`binding in raw_binding_map` または `binding_parts[0] in raw_binding_map`、いずれも `&` 込みの生文字列と比較）に一度も合致せず**全項目が発動不能のまま放置されていた**と判明（〈Phantom Sigil Pruning II〉2026-05-26 で dead alias を剪定した際も見逃されていた同根の不具合）。実害は二種——① SIGN レイヤーの `&kp LS(NUMBER_5)` 等が既定の `Sft+5` 表記のまま描画され記号そのものが見えない、② `&smart_num` `&mkp MB1` `&mmv` 等のカスタムbehaviorが `&smart_num`/`16` のような生の2段表記に落ちていた。対処として全項目に `&` を付与、かつ SIGN レイヤーで使われる `&kp LS(NUMBER_X)` 系（`%` `!` `#` `$` `&` `{` `}` `~` `"` `?` `(` `)` `|` `+` `*` `<` `>` `_` `:` の19種）を実グリフへの直接マッピングとして新規追加。一方 `gesture_mo_kp`（`&`無しの汎用キー）は削除——これは `zmk,behavior-hold-tap` として既に自動認識されタップ文字＋階層名で正しく描画されており、`&`を付けて「復活」させると全キーが `Mo/Kp` の汎用ラベルに退行するため（`mouse_gesture_kp`/`mouse_gesture_mkp`/`rotate`/`swapper`/`swapper_rev` も同様の理由で、既存の未使用状態のまま温存し変更なし）。修正はローカルに `keymap-drawer` を導入し実際に `keymap parse`/`draw` を実行して検証済み（`keymap.yaml`/`keymap.svg` は本コミットで同梱の最新版に同期、通常は `draw-keymap.yml` が main push 時に自動再生成する経路と同一処理）。 |
| 2026-09-19 | 〈Conduit Blackout Resealing〉— ユーザー報告「ライブ機能が使えなくなった」を受けて調査。原因は2つ複合していた。①`https://cardinal-sys.github.io/Release_Recollection_Cardinal/` 配下（`live.html` 含む）が **404** で応答不能——GitHub Actions API で確認したところ `deploy-editor.yml` の実行履歴が `total_count: 0`（一度も実行されていない）、かつ全ワークフローの `created_at` が同一時刻に揃っており、直近のリポジトリ再構築で GitHub Pages の初期状態（Settings → Pages → Source）が引き継がれていなかったと判明。`workflow_dispatch` で手動実行したところ `actions/configure-pages@v5` が `Create Pages site failed: Resource not accessible by integration` で失敗——Pages サイト自体が未作成で、`GITHUB_TOKEN` には新規作成権限がないため。Settings → Pages → Source を "GitHub Actions" に手動設定後、再実行で `success` となり `live.html` の 404 も解消（実機確認済み）。②Cardinal Editor 本体の〈Sealing〉（Seal & Commit）が `POST /repos/.../git/blobs` で 404 失敗——`GET` 系（ファイル読込）は成功するのに書き込み系のみ失敗する典型パターンで、保存済み PAT の対象リポジトリへの書き込み権限（Contents: Read and write）が失効/不整合と判断。**🔑 Change PAT** から PAT を再発行し解消（ユーザー確認済み、以後 Live Sync Conduit 経由のキーマップ同期・コミットとも正常動作）。 |
| 2026-09-18 | 〈True Name Succession〉— 〈Self-Governance Oath〉で用意した新本体リポジトリ `cardinal-sys/Cardinal-v2` への移行が完了し、GitHub上でリポジトリ名を `Cardinal-v2` → `Release_Recollection_Cardinal` へ改名。従来`origin`だった旧本体（保険として温存していた個体）は名称衝突回避のため `cardinal-sys/Release_Recollection_Cardinal_V2` へ改名し役目を退いた。42キー版の通常使用リポジトリは、改名後の `cardinal-sys/Release_Recollection_Cardinal`（本リポジトリ）に一本化。GitHubのリポジトリ改名はURLリダイレクトが効くため、既存クローンの`origin`設定に変更は不要。 |
| 2026-09-18 | 〈Artifact True Name〉— `build.yaml` の成果物ファイル名（`artifact-name`）を剣の真名だけに統一。1号機central/peripheralに未設定だった`artifact-name`を明示追加（`Elucidator`/`Dark_Repulser`）、2号機・3号機の`Elucidator_NightSkySword`/`Elucidator_Excalibur`から冗長な`Elucidator_`接頭辞を除去し`NightSkySword`/`Excalibur`に短縮。シールド定義・Kconfig・`CONFIG_ZMK_KEYBOARD_NAME`（BLE表示名）は変更なし——成果物ファイル名の見た目のみの調整。`Release_Recollection_Cardinal`側と同一の変更（コミット `c67f3d9`）をこちらにも反映。 |
